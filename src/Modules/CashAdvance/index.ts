...
import { CashAdvanceDetailType, CashAdvanceQuery, CashAdvanceType, financeActionCashAdvance, hrActionCashAdvance, managerActionCashAdvance } from "./types";
...
export class CashAdvanceModule {
	...
	hrActionCashAdvance = (data: hrActionCashAdvance, id: number) => {
		...
		/**
		 * Update Order
		 * @param {managerActionCashAdvance} data - Order Number
		 * @param {number} id - Record Id
		 */
	managerCashAdvance = (data: managerActionCashAdvance, id: number) => {
		return apiInstance.patch(BASE_URL + this.endPoint + `/managerAction/${id}`, data);
	}
