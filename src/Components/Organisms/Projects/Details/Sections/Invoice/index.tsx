...
const ProjectInvoice: FC<ProjectInvoiceProps> = ({ projectId }) => {
  ...
  const [xeroInvoices, setXeroInvoices] = useState<InvoiceTypes[] | undefined>(undefined);
  ...
  // Fetch Xero invoices and handle errors
  useEffect(() => {
    const fetchXeroInvoices = async () => {
      try {
        const response = await module.getAllRecords({
          // __status: [InvoiceStatus.AUTHORIZED], // Pass as an array
          id: 0,
          projectId: 0,
          clientId: 0,
          projectTypeId: 0,
        ...
        // Check if response.data is an array of InvoiceTypes
        if (Array.isArray(response.data)) {
          setXeroInvoices(response.data);
        } else {
          console.error('Response data is not an array of InvoiceTypes:', response.data);
        }
      } catch (error) {
        console.error('Failed to fetch invoices from Xero:', error);
      }
    };
  
    fetchXeroInvoices();
  }, []);
  // Ensure xeroInvoices is an array, even if undefined
  const xeroInvoicesArray: InvoiceTypes[] = xeroInvoices ?? [];

  // Combine data from your system and Xero
  const combinedInvoices = [...(data ?? []), ...xeroInvoicesArray];
