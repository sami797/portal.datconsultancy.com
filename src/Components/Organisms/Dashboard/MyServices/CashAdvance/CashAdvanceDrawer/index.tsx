...
export const CashAdvanceDrawer = ({ openDrawer, onClose, record, permissions, reloadTableData }: PropTypes) => {
  ...
  const { updateCashAdvance, cashAdvanceHRApproval, cashAdvanceFinanceApproval ,cashAdvanceMANAGERApproval} = permissions;
  ...
  const onFinish = (formValues: { comment: string, status: CashAdvanceDetailType["status"], approvedAmount?: number, numberOfInstallments: number }) => {
    ...
    if (updateCashAdvance || cashAdvanceHRApproval || cashAdvanceFinanceApproval || cashAdvanceMANAGERApproval) {
      //means hr action
      ...
      <div key={index} className={styles.attachment}>
        <div className={styles.resourceWrap}>
          {attachment.mimeType?.includes("pdf") ? (
            <a
              href={`${BASE_URL}resources/all/${attachment.file}?authKey=${access_token}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              {/* Small preview rendered inline */}
              <Document
                file={{
                  url: `${BASE_URL}resources/all/${attachment.file}?authKey=${access_token}`,
                }}
              >
          ...
          <div style={{ padding: "0.25rem 0.5rem", borderRadius: "0.25rem", display: "flex", flexDirection: "column" }}>
            <Text style={{ width: 125, fontSize: "var(--font-size-xs)" }} strong ellipsis={true}>
              {attachment.title}
            </Text>
          </div>
        </div>
            ...
            {data?.status !== CashAdvanceEnum.withdrawn && (
      ...
      {!data?.AdminActions.find(action => action.Department.slug === "hr") ? (
        ...
        <div>
          <div className={styles.actionsWrap}>
            ...
            >
              AED {data?.requestAmount}
            </div>
            {data?.approvedAmount !== data.requestAmount && (
              <div className={styles.approvedAmount}>AED {data?.approvedAmount}</div>
            )}
          </div>
        </div>
      )}
    ...
    <div className={styles.footerWrap}>
      {!data?.AdminActions.find(action => action.Department.slug === "hr") ? (
        ...
        <div className={styles.priceText}>
          ...
          <div>
            {data?.AdminActions.find(action => action.Department.slug === "hr")?.comment}
          </div>
        </div>
      )}
      {!data?.AdminActions.find(action => action.Department.slug === "hr") ? (
...
{/* Render Manager Actions only if HR has approved */}
{data?.AdminActions.find(action => 
  action.Department.slug === "hr" && 
  action.status === CashAdvanceEnum.approved // Numeric value
) && (

  <div className={styles.hrWrap}>
    <div className={styles.title}>MANAGER Actions</div>
    <div className={styles.hrInputs}>
      {!data?.AdminActions.find(action => action.Department.slug === "manager") ? (
        cashAdvanceHRApproval ? (
          <div>
            <b style={{ fontSize: "var(--font-size-sm)" }}>Amount</b>
            <Form.Item
      ...
      <div className={styles.actionsWrap}>
        ...
        {!data?.AdminActions.find(action => action.Department.slug === "manager") ? (
          ...
          <div style={{ textTransform: "capitalize", lineHeight: "1rem" }}>
            {/* @ts-ignore */}
            {CashAdvanceStatus[data?.AdminActions?.find(action => action.Department.slug === "manager")?.status]?.status}
          </div>
        )}
      </div>
    </div>
    <div className={styles.footerWrap}>
      {!data?.AdminActions.find(action => action.Department.slug === "manager") ? (
        ...
        <div className={styles.priceText}>
          <b style={{ fontSize: "var(--font-size-sm)" }}>Comment</b>
          <div>
            {data?.AdminActions.find(action => action.Department.slug === "manager")?.comment}
          </div>
        </div>
      )}
      {!data?.AdminActions.find(action => action.Department.slug === "manager") ? (
        cashAdvanceHRApproval && (
          <div className={styles.buttonWrap}>
            ...
            {(data?.status !== CashAdvanceEnum.withdrawn && data?.AdminActions.find(action => action.Department.slug === "manager") && data.status !== CashAdvanceEnum.rejected) ?
                  ...
                  {!data?.AdminActions.find(action => action.Department.slug === "finance") ?
                    (cashAdvanceFinanceApproval && <div>
                      ...
                      <Form.Item name="numberOfInstallments">
                        <Slider marks={marks} defaultValue={2} min={1} max={4} />
                      </Form.Item>
                    </div>) : <div>
                      <Divider />
                      <b>Installments:</b>
