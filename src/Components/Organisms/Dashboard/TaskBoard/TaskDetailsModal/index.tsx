...
import { SortDownIcon, XMarkIcon } from "@icons/";
...
const handleDownload = (url: string, filename: string) => {
  fetch(url)
    ...
    .then(blob => {
      // Ensure the blob is treated as a PNG file
      const pngBlob = new Blob([blob], { type: 'image/png' });
      const link = document.createElement('a');
      const objectURL = URL.createObjectURL(pngBlob);

      link.href = objectURL;
      link.download = filename.endsWith('.png') ? filename : `${filename}.png`;  // Ensure the filename ends with .png
...
const handleDownloadImage = (path: string, name: string) => {
  // Construct the URL with the access token
  const access_token = TokenService.getLocalAccessToken();
  const url = `${BASE_URL}resources/all/${path}?authKey=${access_token}`;
...
const { data, onRefresh, setData } = useFetchData<TaskDetailType>({
  ...
  useEffect(() => {
    // Set the current user's ID when the component mounts
    const user = TokenService.getLoggedInUser();
    setCurrentUserId(user?.id || null);
  }, []);
   
  const isAssignee = useMemo(() => {
    return data?.TaskMembers.some(member => member.User.id === currentUserId);
  }, [data, currentUserId]);
  const canEditTaskName = useMemo(() => {
    return data?.AddedBy.id === currentUserId;
  }, [data, currentUserId]);
  ...
  <b>Task Name</b>
  {canEditTaskName && isUpdate.taskTitle.show ? (
    <EditableInput
      onBlur={() => {
      ...
      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
        setIsUpdate({ ...isUpdate, taskTitle: { ...isUpdate.taskTitle, value: e.target.value } })
      }
    ...
    <div
      onClick={() => {
        // Only allow editing if the user is either the assignee or the one who added the task
        if (canEditTaskName || isAssignee) {
          setIsUpdate({ ...isUpdate, taskTitle: { ...isUpdate.taskTitle, show: true } });
        }
      }}
  ...
  <>
    <div className={styles.title}>
      <b>Attachments ({data?.Resources.length})</b>
    </div>
      ...
      {data.Resources.map((resource) => (
        <div key={resource.id} className={styles.resourceWrap}>
          {resource.fileType.includes("image") ? (
            <a
              href={`${BASE_URL}resources/all/${resource.path}?authKey=${access_token}`}
              target="_blank"
          ...
          >
            <Text style={{ width: 125, fontSize: "var(--font-size-xs)" }} strong ellipsis>
              {resource.name}
            </Text>
            <Text style={{ width: 125, fontSize: "var(--font-size-xxs)" }} ellipsis>
              {convertDate(resource.addedDate, "dd M,yy-t")}
            </Text>
          </div>
          <Popconfirm
          ...
          <div style={{ display: "flex", justifyContent: "center" }}>
            <Button
              onClick={() =>
                handleDownload(
                  `${BASE_URL}resources/all/${resource.path}?authKey=${access_token}`,
                  resource.name
                )
              }
