...
import { ClientStatus, ClientsType } from "@helpers/commonEnums";
const { Option } = Select;
...
export const ClientModal = (props: ClientModalProps) => {
  ...
  const onFinish = (formValues: ClientType) => {
    ...
    const { phoneCode, phone, status, ...rest } = formValues
    const finalStatus = status === ClientStatus.ACTIVE ? ClientStatus.ACTIVE : ClientStatus.INACTIVE;
    switch (type) {
      case "edit": {
        if (updateClient === true) {
          module.updateRecord({
            ...
            status: finalStatus // Use enum value
          }, record).then((res) => {
            reloadTableData();
            onCancel();
