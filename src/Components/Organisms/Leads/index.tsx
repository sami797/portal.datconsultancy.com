...
import {
  CustomFilter, CustomInput, ExcelExport, PageHeader, Pagination
} from "@atoms/";
...
import LeadsTable from "./table-columns";
import AddNoteModal from "./PopModel";
...
const Leads: FC = () => {
  ...
  const [selectedRecord, setSelectedRecord] = useState<LeadsTypes | null>(null);
  ...
  const reloadTableData = () => {
    // Implement the logic to reload the table data
  };
 
  const handleNoteSave = async (note: string, isConcern: boolean) => {
    if (selectedRecord) {
      const recordId = selectedRecord.id;
    
      try {
        // Call the API to save the note using leadsModule instance
        const response = await leadsModule.addNote(recordId, { note, isConcern });
    
        // Handle successful response
        console.log('Note saved successfully:', response);
        message.success('Note saved successfully');
    
        // Optionally, you might want to update the UI or perform additional actions
        ...
        // const errMessage = error.response?.data?.message || 'Error saving note'; // Define errMessage here
        // message.error(errMessage);
      }
    }
  };
  
  const handleAddNoteClick = (record: LeadsTypes) => {
  ...
  const handleNoteSaveWrapper = (note: string) => {
    // Default value for isConcern
    const isConcern = false; // Adjust based on your logic or UI state
    handleNoteSave(note, isConcern);
  };
  ...
  const onUpdate = useCallback((query?: QueryType<LeadsParamTypes>) => {
    ...
    const toDate = toDateString ? convertDate(toDateString, "yy-mm-dd") : undefined;
     const data: QueryType<LeadsParamTypes> = {
      ...
      // assignedToId: debouncedKeySearchTerm || undefined,
      ...
      hasConcerns: selectedFilters.hasConcerns || undefined,
       ...
       projectTypeTitle: debouncedTitleSearchTerm || selectedFilters.projectTypeTitle || undefined,
     
      // searchterm : selectedFilters.search || undefined
      ...query,
    }
    ...
    onRefreshStatusCount();
  }, [selectedFilters, debouncedTitleSearchTerm])
  ...
  useEffect(() => {
    onUpdate({ projectTypeTitle: debouncedTitleSearchTerm });
}, [debouncedTitleSearchTerm, onUpdate]);
 ...
 <div className={styles.antdTableWrapper}>
  {projectViewAs === "grid" ? (
    ...
    <LeadsTable
      ...
      onEditIconClick={(record) => onEditIconClick(record.id)}
