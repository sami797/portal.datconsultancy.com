...
import QuotationsTable from "./table-colum";
...
const useFetchXeroQuotations = () => {
  const [xeroQuotations, setXeroQuotations] = useState<QuotationTypes[]>([]);
  ...
  useEffect(() => {
    const fetchXeroQuotation = async () => {
      setLoading(true);
      try {
        const response = await apiInstance.get(`/xero/quotations?page=${page}&pageSize=100`);
        console.log("Xero Projects Response:", response.data);
        
        // Add newly fetched quotations to the existing state
        setXeroQuotations(prevQuotations => [
          ...prevQuotations,
          ...(response.data.data || []),
        ]);

        // Set total pages if the response includes it
  ...
  return { xeroQuotations, loading, error, totalPages, setPage };
};
...
const Quotations: FC = () => {
  const [combinedQuotations, setCombinedQuotations] = useState<QuotationTypes[]>([]);
  ...
  const { xeroQuotations, loading: xeroLoading, error: xeroError } = useFetchXeroQuotations();
  ...
  const statusValues = {
    ...
    draftn: [QuotationStatus.Pending],
  ...
  console.log('Initial Query:', { __status: statusValues.active });
  
  // Log data, meta, and loading status from the API
  useEffect(() => {

  }, [data, meta, loading]);
  useEffect(() => {
    let combinedData: QuotationTypes[] = [];
  
    // Combine quotations from Xero and other sources
    if (data && Array.isArray(data) && xeroQuotations && Array.isArray(xeroQuotations)) {
      combinedData = [...data, ...xeroQuotations];
    } else if (data && Array.isArray(data)) {
      combinedData = data;
    } else if (xeroQuotations && Array.isArray(xeroQuotations)) {
      combinedData = xeroQuotations;
    } else {
      combinedData = [];
    }
  
    // Update the combined quotations state
    setCombinedQuotations(combinedData);
  }, [data, xeroQuotations]);
  ...
  const segmentedOptions = [
    ...
    { label: "Pending Draft", value: "draftn" },
  ]
  ...
  return (
    ...
    >
      ...
      onChange={(value) => {
        ...
        localStorage.setItem("projectViewAs", value as ProjectViewAsType);
      }}
    />
            </div>
          ) : null
  ...
  <>
    {projectViewAs === "grid" ? (
      <QuotationsCard
        ...
        data={{
          allQuotation: combinedQuotations || [],  // Use the combined quotations
          onRefresh: () => onUpdate({
    ...
    <Pagination
      total={meta?.total || combinedQuotations.length}  // Use combined quotations length for total count
      current={meta?.page || 1}
      defaultPageSize={meta?.perPage || 20}
      pageSizeOptions={[10, 60, 80, 400]}
      onChange={onPaginationChange}  // Handle page changes
    />
  </>
)}
