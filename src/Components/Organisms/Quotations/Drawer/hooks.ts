...
export const usePullFromXero = (props: UsePullFromXeroProps) => {
  ...
  const onPullFromXero = () => {
    ...
    const { xeroTenantId } = pullFromXero; // Only using tenantId to fetch all quotes

    // Fetch all quotations for the tenant
    getQuotes({ tenantId: xeroTenantId }).then((res) => {
      const data = res?.data?.data;

      // Ensure data is an array of quotations before proceeding
      if (Array.isArray(data)) {
        setPreviewData(data); // Set all fetched quotes in previewData

        // Optionally, you can set the drawer to preview mode
        setDrawer((prev) => ({
          ...prev,
          type: "preview",
          quotes: data, // Pass all fetched quotes to the drawer
        }));
      } else {
        message.warning("No quotations found.");
      }
    }).catch((err) => {
      const errorMessage = handleError(err);
      message.error(errorMessage || "Something went wrong");
